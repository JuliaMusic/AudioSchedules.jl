...
const EXPONENT_INTERPOLATOR = CubicSplineInterpolation(
    REFERENCE_LOG_10_FREQUENCIES,
    [
        0.532,
        0.506,
        0.480,
...
const LINEAR_INTERPOLATOR = CubicSplineInterpolation(
    REFERENCE_LOG_10_FREQUENCIES,
    [
        -31.6,
        -27.2,
...
const THRESHOLD_INTERPOLATOR = CubicSplineInterpolation(
    REFERENCE_LOG_10_FREQUENCIES,
    [
        78.5,
        68.7,
...
function equivalent_sound_pressure(phon, frequency)
    ...
    A_f =
        4.47e-3 * (10^(0.025 * phon) - 1.15) +
        (0.4 * (10^((threshold + linear) / 10 - 9)))^exponent
    ...
    Map(
        let multiplier =
                equivalent_sound_pressure(
                    MEDIUM_LOUDNESS,
                    (synthesizer.synthesizers[1].frequency),
                ) / MAXIMUM_PRESSURE
            function (amplitude)
